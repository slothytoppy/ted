...
FileViewerRendererType :: events.Renderer(FileViewer)
FileViewerRenderer :: FileViewerRendererType {
	init   = InitFileViewer,
	update = update,
	render = render,
}
...
@(require_results)
InitFileViewer :: proc() -> (fv: FileViewer) {
	return fv
}

update :: proc(file_viewer: ^FileViewer, event: events.Event) -> events.Event {
...
render :: proc(file_viewer: FileViewer) -> []string {
	/*
	switch event {
	case .render_whole:
		ncurses.erase()
